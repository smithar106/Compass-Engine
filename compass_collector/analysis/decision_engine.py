...
class DecisionEngine:
    ...
    def _select_solution_pattern(self, c: InterventionCandidate, workflow: str) -> Optional[SolutionPattern]:
        """Select the best solution pattern for an intervention based on assessment inputs."""
        a = self.assessment
        family_id = c.family_id
        candidates = [p for p in SOLUTION_PATTERNS.values() if p.family_id == family_id]
        if not candidates:
            return None
    ...
    def _generate_executive_title(self, workflow: str, family_id: str) -> str:
        """Generate a CEO-readable executive title from workflow + intervention family.
        Never includes technology names unless essential to the decision itself."""
        wf = workflow.replace("_", " ").title() if workflow else "This Workflow"

        titles = {
            "Workflow_Automation": f"Automate {wf}",
            "AI": f"Automate {wf}",
            "Software": f"Implement Software for {wf}",
            "Process_Redesign": f"Redesign {wf}",
            "Staffing": f"Add Capacity to {wf}",
            "Hybrid": f"Improve {wf}",
            "No_Action": f"Establish Baseline for {wf}",
        }
        return titles.get(family_id, f"Improve {wf}")

    def _generate_operating_model_change(self, workflow: str, family_id: str) -> str:
        """Describe how work changes — the structural shift, not the technology."""
        wf = workflow.replace("_", " ").lower() if workflow else "this workflow"

        changes = {
            "Workflow_Automation": f"Move {wf} from manual processing to automated routing with exception-based review.",
            "AI": f"Move {wf} from manual handling to AI-assisted processing with human escalation for complex cases.",
            "Software": f"Move {wf} from manual or spreadsheet-based work to a purpose-built software solution.",
            "Process_Redesign": f"Redesign {wf} to eliminate unnecessary steps and clarify ownership before introducing new technology.",
            "Staffing": f"Add dedicated capacity to {wf} through additional staffing rather than technology investment.",
            "Hybrid": f"Combine automation for routine {wf} with human expertise for exceptions and high-judgment cases.",
            "No_Action": f"Do not invest in new technology for {wf}. Establish the current operating baseline first.",
        }
        return changes.get(family_id, f"Improve {wf} through the recommended intervention.")

    def _generate_implementation_pattern(self, pattern: Optional[SolutionPattern]) -> str:
        """Describe the implementation approach — how it might be accomplished."""
        if not pattern:
            return "Implement the recommended approach with a bounded pilot."
        return pattern.description

    def _format_candidate(self, c: InterventionCandidate) -> dict:
        workflow = (self.assessment.workflow or self.assessment.problem_statement or "")
        pattern = self._select_solution_pattern(c, workflow)
        executive_title = self._generate_executive_title(workflow, c.family_id)
        operating_model = self._generate_operating_model_change(workflow, c.family_id)
        impl_pattern = self._generate_implementation_pattern(pattern)
        ...
        return {
            ...
            "operating_model_change": operating_model,
