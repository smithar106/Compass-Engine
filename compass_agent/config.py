...
class Settings:
    ...
    max_daily_llm_usd: float = 2.50
    max_total_llm_usd: float = 10.00
    llm_concurrency: int = 4
    max_docs_per_cycle: int = 20
    sleep_seconds: float = 600
    ...
    def from_env(cls, env: dict | None = None) -> "tuple[Settings, list[str]]":
        ...
        settings = cls(
            ...
            max_daily_llm_usd=_num("AGENT_MAX_DAILY_LLM_USD", 2.50),
            max_total_llm_usd=_num("AGENT_MAX_TOTAL_LLM_USD", 10.00),
            llm_concurrency=_int("AGENT_LLM_CONCURRENCY", 4),
            max_docs_per_cycle=_int("AGENT_MAX_DOCS_PER_CYCLE", 20),
            sleep_seconds=_num("AGENT_SLEEP_SECONDS", 600),
            auto_publish=_parse_bool(env.get("AGENT_AUTO_PUBLISH"), False),
            state_file=str(env.get("AGENT_STATE_FILE") or "").strip(),
            store_db=str(env.get("AGENT_STORE_DB") or "").strip(),
